...
$(document).ready(function() {
	...
	$('#btn_nxt_user').click(function() {
	    ...
	    if($('#frm_user').valid()){	    	
		    ...
		    if($('#role_id').val()=='1'){
		    	...
		    	$('#btn_acc_cont').hide();	    		
	    		loaderOn();	    		
	    		$.post('jsp/echosign.jsp?t=' + Math.random() + '&signtype=one&doc=/docs/listingagreement.pdf&userid=' + userid + '&email1=' + $('#email').val(), function(data,status){	
	    		...
	    		//** temporary disabled
				//var chkechodoc1 = setInterval(function(){$.post('jsp/echosigncheck.jsp?t=' + Math.random(),function(data,status){
		    	...
		    	if($('#investor_type').val()=='U'){
		    		...
		    		$('#btn_acc_cont').hide();
					$.post('jsp/echosign.jsp?t=' + Math.random() + '&signtype=one&doc=/docs/checklist_accinvestor.pdf&userid=' + userid + '&email1=' + $('#email').val(), function(data,status){	
						$('#frame_pptosnda').attr('src',$.trim(data));	    			
						mdlppShow();
						loaderOff();							    			
					});
		    		...
		    		//setInterval(function(){$.post('jsp/echosigncheck.jsp?t=' + Math.random(),function(data,status){			    			
		    		//		if(data.match("download success")){			    				
		    					$('#btn_acc_cont').show();
		    		//		}
		    		//	});},3000);			    		
	...
	$('#btn_nxt_project').click(function() {
		...
		$inputs.each(function() {
			if($(this).attr('id').match('target_amount')) $(this).val().replace('$','').replace(',','').replace(',','').replace(',','');
			...
			<fieldset>
				...
				<form id='frm_project'>
					...
					<br> <span class="help-inline">Image 2</span> <input
						...
						placeholder='Image 3'>  -->					
					<br> <input type='text' id='target_amount' name='target_amount'
						class='input-medium' placeholder='Target Amount' required number="true" min="1000" max="1000000"> 
				</form>
			</fieldset>
		</div>
		<div class="modal-footer">
